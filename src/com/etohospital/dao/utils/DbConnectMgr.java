...
public class DbConnectMgr {
	...
	// 唯一实例
	static private DbConnectMgr instance;
	 ...
	 */
	private DbConnectMgr() {
		init();
	}
		
	private DbConnectMgr(LinkedList<String> drivers, Hashtable<String, DbInfo> jdbcs) {
		init2(drivers, jdbcs);
	}
	 ...
	 */
	public static synchronized DbConnectMgr getInstance() {
			
		if (instance != null) {
			clients++;
			return instance;
		} else {
			instance = new DbConnectMgr();
			return instance;
		}
	}
	 ...
	 * @param DbInfo	连接数据库的参数
	 * @return
	 */
	public static synchronized DbConnectMgr getInstance(DbInfo DbInfo) {
		...
		LinkedList<String> drivers = new LinkedList<String>();
		drivers.add(DbInfo.getDriver());
		Hashtable<String, DbInfo> jdbcs = new Hashtable<String, DbInfo>();
		jdbcs.put(DbInfo.getName(), DbInfo);
	 ...
	 * @return
	 */
	public static synchronized DbConnectMgr getInstance(LinkedList<String> drivers,	Hashtable<String, DbInfo> jdbcs) {
			
		if (instance == null) {
			instance = new DbConnectMgr(drivers, jdbcs);
		}
	...
	private void init() {
		...
		try {
			...
			System.err.println("不能够读取默认数据库连接池配置文件，请确认文件是否存在：" + DEFAULT_DB_PROPERTIES);
			return;
		}
	 ...
	 * 创建多个数据库连接池
	 * @param drivers	每个连接池的数据库驱动类
	 * @param DbInfo	每个连接池的连接参数
	 */
	private void init2(LinkedList<String> drivers, Hashtable<String, DbInfo> DbInfo) {
	 ...
	 * @param mdrivers
	 */
	private void loadDrivers(LinkedList<String> mdrivers) {
		
		for (int i = 0; i < mdrivers.size(); i++) {
					
			try {
				//根据数据库连接驱动类，利用反射机制创建驱动类对象
				Driver driver = (Driver) Class.forName(mdrivers.get(i)).newInstance();
				DriverManager.registerDriver(driver);
				drivers.addElement(driver);
				System.out.println("成功加载数据库连接驱动:  " + mdrivers.get(i));
			} catch (Exception e) {
				System.err.println("加载数据库连接驱动失败:  " + mdrivers.get(i) + ". 错误信息：" + e);
			}
		}
	}
	...
	private void loadDrivers(Properties props) {
		...
		while (st.hasMoreElements()) {
			...
			try {
				...
				DriverManager.registerDriver(driver);
				drivers.addElement(driver);
				System.out.println("成功加载数据库连接驱动: " + driverClassName);
			} catch (Exception e) {
				System.err.println("加载数据库连接驱动失败: " + driverClassName + ". 错误信息: " + e);
			}
		}
	}
		...
		while (propNames.hasMoreElements()) {
			...
			if (name.endsWith(".url")) {
				...
				try {
					...
					System.err.println("最大连接数限制数错误: " + maxconn + " .连接池名: " + poolName);
					max = 0;
				}
	 ...
	 * @param DbInfos
	 */
	private void createPools(Hashtable<String, DbInfo> DbInfos) {

		Iterator<?> it = DbInfos.entrySet().iterator();
			
		while (it.hasNext()) {
			@SuppressWarnings("rawtypes")
			Map.Entry en = (Map.Entry) it.next();
			DbInfo info = (DbInfo) en.getValue();
			if (info.getUrl() == null) {
				continue;
			}
				
			System.out.println("准备创建数据库连接池: " + (String) en.getKey());
			DBConnectionPool pool = new DBConnectionPool((String) en.getKey(), info.getUrl(), info.getUser(), info.getPassword(), info.getMaxconn());
			pools.put((String) en.getKey(), pool);
		}
	}
	...
	class DBConnectionPool {
		...
		private Vector<Connection> freeConnections = new Vector<Connection>();	//该连接池中可用的数据库连接
		...
		private Connection newConnection() {
			...
			try {
				...
				System.out.println("连接池" + this.name + "创建一个新的数据库连接, 目前共有" + this.checkedOut + "个连接在使用！");
			} catch (SQLException e) {
